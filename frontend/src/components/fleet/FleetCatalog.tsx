...
import type { FleetFilters, SortOption, Vehicle, VehicleCategory } from '@/types/vehicle';
import { DEFAULT_FLEET_FILTERS } from '@/types/vehicle';

const PER_PAGE = 9;

export interface FleetCatalogProps {
  vehicles: Vehicle[];
  loading?: boolean;
  /** e.g. `/agency/inova-ride/fleet` — detail links become `{basePath}/{id}` */
  fleetBasePath?: string;
  /** e.g. `/agency/inova-ride/booking` — book links append ?vehicleId= */
...
export function FleetCatalog({
  ...
  const filtered = useMemo(() => {
    const list = filterVehicles(vehicles, appliedFilters);
    return sortVehicles(list, sort);
  }, [vehicles, appliedFilters, sort]);
        ...
        <div className="min-w-0 flex-1">
          <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
            <p className="font-display text-sm text-text-muted">
              {loading ? '…' : t('found_count', { count: total })}
            </p>
            <select
              value={sort}
              onChange={(e) => {
          ...
          {loading ? (
            ...
            <>
              ...
              />
              <FleetPagination page={safePage} totalPages={totalPages} onPageChange={setPage} />
            </>
          )}
        </div>
      </div>
